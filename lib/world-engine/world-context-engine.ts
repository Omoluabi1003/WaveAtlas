...
export async function getWorldContext(input: WorldContextInput): Promise<WorldContext> {
  ...
  const sources: SourceResult[] = settled.map((item, index) => item.status === "fulfilled" ? item.value : { status: "error", source: ["RestCountries", "Wikipedia", "USGS Earthquake", "OpenStreetMap", "GeoNames"][index], confidence: 0, data: null, attribution: "Open data", error: item.reason instanceof Error ? item.reason.message : "Unavailable" });
