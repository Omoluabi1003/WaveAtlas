...
export function rankNearbyStations(stations: Station[], focus: GeoFocus, limit = 5): RankedStationCandidate[] {
  ...
  return stations
    .filter((station) => station.is_active && station.failure_count <= 2 && Boolean(station.url_resolved || station.url) && station.country_code !== 'UN')
    .map((station) => {
      ...
      const score = distanceScore + Math.min(24, station.health_score * 0.24) + (station.last_check_ok ? 10 : 0) + Math.min(9, station.bitrate / 24) + Math.min(10, station.votes / 2500) + Math.min(5, metadataQuality(station));
