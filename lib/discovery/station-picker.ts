...
export function scoreStation(station: Station, history: ArrivalHistory, mode: StartupMode) {
  ...
  return station.url && station.is_active ? score : 0;
}
export function pickStation(stations: Station[], history: ArrivalHistory, mode: StartupMode, filter?: (station: Station) => boolean) { const pool = Math.random() < 0.1 ? stations.filter((station) => hiddenGemCountries.has(station.country_code)) : stations; const candidates = pool.filter((station) => station.url && station.is_active && (!filter || filter(station))); const valid = candidates.filter((station) => stationHistoryKey(station) !== history.last100Stations[0]); return weightedPick(valid.map((item) => ({ item, weight: scoreStation(item, history, mode) }))) ?? weightedPick(candidates.map((item) => ({ item, weight: 1 }))); }
