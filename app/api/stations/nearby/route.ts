...
export async function GET(req: NextRequest) {
  ...
  const globalStations = globalTeleport ? await fetchGlobalCandidateStations(4) : countryStations.length ? [] : await fetchStations({ limit: '120', allowFallback: 'false' });
  ...
  const candidates = globalTeleport
