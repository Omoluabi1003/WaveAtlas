...
export function TuningReticle({ state, strength }: { state: EarthTunerState; strength: number }) {
  const label = state === 'locked' ? 'Locked' : state === 'signal_found' ? 'Signal Found' : state === 'weak_signal' ? 'Weak Signal' : state === 'no_signal' ? 'No Signal' : state === 'scanning' ? 'Scanning' : 'Idle';
  return <div className="pointer-events-none absolute left-1/2 top-1/2 z-30 -translate-x-1/2 -translate-y-1/2 text-center">
    <motion.div className={`grid size-28 place-items-center rounded-full border ${state === 'no_signal' || state === 'weak_signal' ? 'border-amber-300/45' : 'border-radio/55'} bg-slate-950/15 shadow-[0_0_70px_rgba(88,225,132,.24)] backdrop-blur-[2px]`} animate={{ scale: state === 'scanning' ? [1, 1.08, 1] : 1 }} transition={{ repeat: state === 'scanning' ? Infinity : 0, duration: 1.1 }}>
