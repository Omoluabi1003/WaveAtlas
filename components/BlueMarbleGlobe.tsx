...
type GlobePoint = {
  ...
  geoSource?: string;
  geoPrecision?: string;
  usedFallbackCentroid?: boolean;
};
type GlobeLabelKind = "active" | "country" | "city" | "station";
type GlobeLabel = GlobePoint & {
  active?: boolean;
  kind?: GlobeLabelKind;
  priority?: number;
};
type GlobeRuntime = {
  currentPoint: GlobePoint | null;
...
type GlobeDebugOverlay = {
  ...
  deltaX: number | null;
  deltaY: number | null;
  usableBounds: { left: number; top: number; right: number; bottom: number };
  canvasCenter: { x: number; y: number };
  rotX: number;
  rotY: number;
  selectedCountry: string | null;
  frontFacing: boolean;
  correctiveFocusRan: boolean;
  d3InputOrder: "[longitude, latitude]";
};
type CanvasSize = {
  cssWidth: number;
  cssHeight: number;
...
type GlobeGeometry = {
  ...
  centerX: number;
  centerY: number;
  usableBounds: GlobeUsableBounds;
};
...
type CloudLayerState = {
  quality: CloudQualityMode;
  cloudStatus: CloudResourceStatus;
  systems: CloudSystem[];
  drift: number;
  driftStatus: CloudDriftStatus;
  lastDriftAt: number;
...
const DEBUG_CLOUDS =
  ...
  process.env.NEXT_PUBLIC_WAVEATLAS_DEBUG_CLOUDS === "true";
...
const CLOUD_SLOW_WINDOWS_BEFORE_DOWNGRADE = 3;
const CLOUD_SYSTEMS_FULL = 58;
const CLOUD_SYSTEMS_REDUCED = 22;
const CLOUD_LOBES_FULL_MIN = 4;
const CLOUD_LOBES_FULL_VARIATION = 6;
const CLOUD_LOBES_REDUCED_MIN = 3;
const CLOUD_LOBES_REDUCED_VARIATION = 3;
const CLOUD_DESKTOP_DRIFT_SPEED = 0.000055;
const CLOUD_MOBILE_DRIFT_SPEED = 0.000018;
const CLOUD_MAX_DRIFT_DELTA_MS = 34;
const CLOUD_DRIFT_NORMALIZE_DEGREES = 360;
...
function drawSpaceBackdrop(
  ...
  const starCount = lowPower
    ? SPACE_STAR_COUNT_LOW_POWER
    : mobile
      ? SPACE_STAR_COUNT_MOBILE
      : SPACE_STAR_COUNT_DESKTOP;
  ctx.save();
  ...
  for (let index = 0; index < starCount; index += 1) {
    ...
    const twinkle = reducedMotion
      ? 0
      : Math.sin(now * 0.00042 + star.phase) * star.twinkle;
    const alpha =
      Math.max(0.04, star.alpha + twinkle) *
      (distanceFromGlobe < radius * 1.22 ? 0.58 : 1);
  ...
  if (!lowPower) {
    ...
    for (const [index, scale] of [1.18, 1.34].entries()) {
      ...
      ctx.strokeStyle =
        index === 0 ? "rgba(125,211,252,0.075)" : "rgba(0,214,143,0.055)";
...
function normalizeLandFeature(feature: NaturalEarthFeature): LandShape | null {
  ...
  const rings: LandRing[] =
    geometry.type === "Polygon"
      ...
      : (geometry.coordinates as number[][][][]).flatMap((polygon) =>
          polygon.map((ring) =>
            ring.map(([lng, lat]) => [lng, lat] as [number, number]),
          ),
        );
  ...
  return {
    ...
    code:
      typeof props.ISO_A2 === "string" && props.ISO_A2.length === 2
        ? props.ISO_A2
        : undefined,
    rings,
...
function resolveCloudQuality(
  profile: ReturnType<typeof getDeviceProfile>,
  iosWebKit: boolean,
  reducedMotion: boolean,
): CloudQualityMode {
  if (!PHOTOREALISTIC_CLOUDS_ENABLED || !profile.webgl) return "disabled";
  if (profile.lowPower || profile.mobile || iosWebKit || reducedMotion)
    return "reduced";
  return "full";
}

function buildCloudSystems(quality: CloudQualityMode): CloudSystem[] {
  if (quality === "disabled") return [];
  const count = quality === "full" ? CLOUD_SYSTEMS_FULL : CLOUD_SYSTEMS_REDUCED;
  return Array.from({ length: count }, (_, index): CloudSystem => {
    const seed = 4100 + index * 47;
    const band = seededUnit(seed + 1) > 0.55 ? 1 : -1;
    const systemKind = seededUnit(seed + 8);
    const lobeCount =
      quality === "full"
        ? CLOUD_LOBES_FULL_MIN +
          Math.floor(seededUnit(seed + 9) * CLOUD_LOBES_FULL_VARIATION)
        : CLOUD_LOBES_REDUCED_MIN +
          Math.floor(seededUnit(seed + 9) * CLOUD_LOBES_REDUCED_VARIATION);
    const baseWidth = systemKind > 0.72 ? 10 : systemKind > 0.38 ? 7 : 4.8;
    const baseHeight = systemKind > 0.72 ? 3.2 : systemKind > 0.38 ? 4.8 : 3.8;
    return {
      lat: Math.max(
        -58,
        Math.min(
          58,
          (seededUnit(seed + 2) - 0.5) * 92 + band * seededUnit(seed + 3) * 18,
        ),
      ),
      lng: seededUnit(seed + 4) * 360 - 180,
      scale:
        quality === "full"
          ? 0.74 + seededUnit(seed + 5) * 0.78
          : 0.58 + seededUnit(seed + 5) * 0.52,
      rotation: (seededUnit(seed + 6) - 0.5) * 1.25,
      alpha:
        quality === "full"
          ? 0.052 + seededUnit(seed + 7) * 0.074
          : 0.038 + seededUnit(seed + 7) * 0.052,
      lobes: Array.from({ length: lobeCount }, (_, lobeIndex): CloudLobe => {
        const lobeSeed = seed + 101 + lobeIndex * 19;
        const side = lobeIndex % 2 === 0 ? 1 : -1;
        const spread = systemKind > 0.72 ? 1.9 : systemKind > 0.38 ? 1.25 : 0.8;
        return {
          x:
            (lobeIndex - (lobeCount - 1) / 2) * spread +
            (seededUnit(lobeSeed + 1) - 0.5) * 2.6,
          y: side * seededUnit(lobeSeed + 2) * (systemKind > 0.72 ? 1.6 : 2.5),
          width: baseWidth * (0.56 + seededUnit(lobeSeed + 3) * 0.72),
          height: baseHeight * (0.58 + seededUnit(lobeSeed + 4) * 0.9),
          alpha: 0.46 + seededUnit(lobeSeed + 5) * 0.54,
          rotation: (seededUnit(lobeSeed + 6) - 0.5) * 0.9,
        };
      }),
    };
  });
...
function resetCloudDriftClock(
  clouds: CloudLayerState,
  now = typeof performance !== "undefined" ? performance.now() : 0,
) {
  clouds.lastDriftAt = now;
}

function advanceCloudDrift(
  clouds: CloudLayerState,
  options: {
    now: number;
    mobile: boolean;
  ...
  if (!canDrift) {
    resetCloudDriftClock(clouds, options.now);
    return;
  }
  ...
  clouds.drift =
    (clouds.drift +
      (options.mobile || clouds.iosReduced
        ? CLOUD_MOBILE_DRIFT_SPEED
        : CLOUD_DESKTOP_DRIFT_SPEED) *
        deltaMs) %
    CLOUD_DRIFT_NORMALIZE_DEGREES;
  resetCloudDriftClock(clouds, options.now);
}

function debugClouds(details: Record<string, unknown>) {
  if (!DEBUG_CLOUDS) return;
  console.debug("[WaveAtlas clouds]", {
    ...details,
...
function surfaceNoise(lat: number, lng: number, seed = 0) {
  return seededUnit(
    Math.round((lat + 90) * 7.13 + (lng + 180) * 3.71 + seed * 101),
  );
}

function projectCanvasGlobePoint(
  lat: number,
  lng: number,
...
function drawPhotorealisticSurface(
  ctx: CanvasRenderingContext2D,
  options: {
    projection: D3GeoProjection;
    cx: number;
  ...
  const bathymetryStep =
    quality === "cinematic" ? 10 : quality === "balanced" ? 15 : 24;
  ...
  for (let lat = -70; lat <= 70; lat += bathymetryStep) {
    for (let lng = -180; lng < 180; lng += bathymetryStep) {
      ...
      const shallow =
        Math.max(0, 1 - Math.abs(lat) / 82) *
        (0.35 + surfaceNoise(lat, lng, 3) * 0.65);
      ctx.fillStyle =
        shallow > 0.72 ? "rgba(103,232,249,0.22)" : "rgba(8,47,73,0.22)";
      ctx.beginPath();
  ...
  for (const shape of landShapes) {
    const elevation =
      0.78 + surfaceNoise(shape.centroid.lat, shape.centroid.lng, 11) * 0.18;
  ...
  for (const shape of landShapes) {
    ...
    ctx.fillStyle =
      arid > 0.58 ? "rgba(190,152,83,0.28)" : "rgba(20,83,45,0.26)";
    ctx.beginPath();
    path(shape.feature);
    ctx.fill("evenodd");
  }
...
function drawPhotorealisticCloudLayer(
  ctx: CanvasRenderingContext2D,
  options: {
    projection: D3GeoProjection;
    clouds: CloudLayerState;
  ...
  const { projection, clouds, now, mobile, reducedMotion, travelActive } =
    options;
  if (
    clouds.quality === "disabled" ||
    clouds.cloudStatus !== "ready" ||
    !clouds.systems.length
  ) {
    clouds.driftStatus = "disabled";
    return;
  }
  advanceCloudDrift(clouds, {
    now,
    mobile,
    reducedMotion,
    travelActive,
  ...
  for (const cloud of clouds.systems) {
    const p = projectCanvasGlobePoint(
      cloud.lat,
      cloud.lng + clouds.drift,
      projection,
    );
    if (p.z < 0.08 || !Number.isFinite(p.x) || !Number.isFinite(p.y)) continue;
    const depth = Math.max(0, Math.min(1, p.z));
    const opacity =
      cloud.alpha * depth * (clouds.quality === "reduced" ? 0.72 : 1);
    const mobileScale = mobile ? 0.72 : 1;
    ctx.save();
    ctx.translate(p.x, p.y);
    ctx.rotate(
      cloud.rotation + Math.sin((cloud.lng + clouds.drift) * DEG) * 0.18,
    );
    for (const lobe of cloud.lobes) {
      ctx.save();
      ctx.translate(
        lobe.x * cloud.scale * mobileScale,
        lobe.y * cloud.scale * mobileScale,
      );
      ctx.rotate(lobe.rotation);
      ctx.fillStyle = `rgba(255,255,255,${opacity * lobe.alpha})`;
      ctx.beginPath();
      ctx.ellipse(
        0,
        0,
        lobe.width * cloud.scale * mobileScale,
        lobe.height * cloud.scale * mobileScale,
        0,
        0,
        TAU,
      );
...
function globeDebugEnabled() {
  return (
    (process.env.NODE_ENV === "development" &&
      process.env.NEXT_PUBLIC_WAVEATLAS_DEBUG_GLOBE === "true") ||
    iosGlobeDebugEnabled() ||
    process.env.NEXT_PUBLIC_WAVEATLAS_DEBUG_TRANSITIONS === "true"
  );
}

function debugGlobeFocus(label: string, details: Record<string, unknown>) {
  if (!globeDebugEnabled()) return;
  console.debug(`[WaveAtlas globe focus] ${label}`, {
    ...details,
    timestamp: new Date().toISOString(),
  });
}
...
function warnGlobeFocus(label: string, details: Record<string, unknown>) {
  if (!globeDebugEnabled()) return;
  console.warn(`[WaveAtlas globe focus] ${label}`, {
    ...details,
    timestamp: new Date().toISOString(),
  });
}
...
function resolveFocusDuration({
  ...
  if (fast)
    return Math.max(
      MOBILE_FAST_FOCUS_DURATION_MS,
      DESKTOP_FAST_FOCUS_DURATION_MS,
    );
  const baseDuration = mobile
    ? MOBILE_PRIMARY_FOCUS_DURATION_MS
    : DESKTOP_PRIMARY_FOCUS_DURATION_MS;
  const scaledDuration = lerp(
    MINIMUM_FOCUS_DURATION_MS,
    LONG_DISTANCE_FOCUS_DURATION_MS,
    easeInOutCubic(distanceRatio),
  );
  return Math.round(Math.max(baseDuration, scaledDuration));
}

function angularDistanceDegrees(
  a: Pick<GlobePoint, "lat" | "lng">,
  b: Pick<GlobePoint, "lat" | "lng">,
) {
...
function angularDistanceBetweenPoints(
  a: GlobePoint | null,
  b: GlobePoint | null,
) {
  if (!a || !b) return null;
...
function stationPoint(station?: Station): GlobePoint | null {
  ...
  const point = {
    ...
    label: beacon.properties.label,
    geoSource: beacon.properties.source,
    geoPrecision: beacon.properties.precision,
    usedFallbackCentroid,
  };
  if (DEBUG_SIGNALS)
    console.debug("[WaveAtlas signals] active beacon coordinates", {
      view: "globe",
      station: station.name,
      lat: point.lat,
      lng: point.lng,
      source: point.geoSource,
      precision: point.geoPrecision,
    });
  debugGlobeFocus("stationPoint", {
    station: station.name,
    city: station.city,
    country: station.country,
    latitude: point.lat,
    longitude: point.lng,
    derivedLat: point.lat,
    derivedLng: point.lng,
    geoSource: geo.source,
    geoPrecision: geo.precision,
    coordinatesFrom: usedFallbackCentroid
      ? "fallback_country_centroid"
      : "station_or_resolved_geo",
    warning: geo.warning,
  });
  return point;
}
...
function landBoundsForShape(shape: LandShape) {
  ...
  const bounds = shape.rings.reduce<LandBounds>(
    (acc, ring) => {
...
function countryResultFromShape(shape: LandShape): CountryResult | null {
  ...
  const centroid =
    isoCountryCentroids[code as keyof typeof isoCountryCentroids];
  return {
...
function nearestCountry(
  ...
  for (const [code, point] of Object.entries(isoCountryCentroids)) {
    ...
    const a =
      Math.sin(dLat / 2) ** 2 +
      Math.cos(lat * DEG) * Math.cos(point.lat * DEG) * Math.sin(dLng / 2) ** 2;
  ...
  return {
    name: countryNameForCode(best.code),
    code: best.code,
    flag: flagFor(best.code),
    centroid,
    station_count: 0,
  };
}
...
function getDeviceProfile() {
  if (typeof window === "undefined" || typeof navigator === "undefined")
    return { lowPower: false, mobile: false, webgl: false, reason: "server" };
  ...
  const lowPower =
    (nav.deviceMemory ?? 8) <= 3 || (nav.hardwareConcurrency ?? 8) <= 4;
  ...
  return {
    ...
    mobile,
    webgl,
    reason: lowPower ? "reduced mobile effects" : "full effects",
  };
}
...
function debugGlobeClick(details: Record<string, unknown>) {
  ...
  console.info("[WaveAtlas globe click]", {
    ...details,
    timestamp: new Date().toISOString(),
  });
}
...
function readGlobeFavoriteSet() {
  ...
  try {
    const parsed = JSON.parse(
      window.localStorage.getItem("waveatlas:favorites") || "[]",
    ) as unknown;
    return new Set(
...
function globeCenterFromRotation(rotX: number, rotY: number) {
  return {
    centerLat: Math.max(-89, Math.min(89, rotX / DEG)),
    centerLng: ((((rotY / DEG) % 360) + 540) % 360) - 180,
  };
}
...
function isGlobePointerBlockedByOverlay(clientX: number, clientY: number) {
  ...
  return selectors.some((selector) =>
    Array.from(document.querySelectorAll(selector)).some((element) =>
      pointInsideRect(clientX, clientY, element.getBoundingClientRect()),
    ),
  );
}
...
function readMobileViewport(wrap: HTMLDivElement, canvas: HTMLCanvasElement) {
  ...
  const usableBounds: GlobeUsableBounds = {
    left: 0,
    top: Math.min(canvasRect.height, topObstruction),
    right: canvasRect.width,
    bottom: Math.max(0, canvasRect.height - bottomObstruction),
  };
  return {
    canvasRect: {
...
export default function BlueMarbleGlobe({
  station,
  stations = [],
  previousStation,
  teleporting = false,
  ...
  const state = useRef({
    ...
    downX: 0,
    downY: 0,
    downOverOverlay: false,
    disabledMotion: false,
    hidden: false,
    focusToken: 0,
    activeFocusToken: 0,
    activeSelectionVersion: selectionVersion,
    verificationPending: false,
    correctiveFocusRan: false,
    verifiedPointKey: "",
    travelActive: false,
    landingPulseStartedAt: 0,
    progressMilestones: new Set<number>(),
    activeFocusIdentityKey: "",
  });
  ...
  const globeLabels = useMemo<GlobeLabel[]>(
    () =>
      currentPoint
        ...
        : [],
    [currentPoint, stationLabel],
  );
  const stationFocusIdentityKey = useMemo(
    () =>
      [
        stationKey(station),
        currentPoint?.lat ?? "none",
        currentPoint?.lng ?? "none",
        selectionVersion ?? "none",
      ].join(":"),
    [currentPoint?.lat, currentPoint?.lng, selectionVersion, station],
  );
  const signalConstellation = useMemo(
    () =>
      buildSignalFeatures({
        stations,
        currentStation: station,
        globeVisibleHemisphere: { centerLat: -10, centerLng: 0 },
        globeScale: 1,
        favoriteIds: readGlobeFavoriteSet(),
        maxSignals: mobile ? 220 : 520,
      }),
    [mobile, station, stations],
  );
  const runtimeRef = useRef<GlobeRuntime>({
    currentPoint,
    selectionVersion,
    focusIdentityKey: stationFocusIdentityKey,
    ...
    labels: globeLabels,
    landShapes,
    signalFeatures: signalConstellation.visibleSignals,
    signalClusters: signalConstellation.clusters,
  });
  ...
  const [debugOverlay, setDebugOverlay] = useState<GlobeDebugOverlay | null>(
    null,
  );
  ...
  const getGlobeGeometryRef = useRef<
    (
  ...
  >(() => ({
    ...
    centerX: 0.5,
    centerY: 0.5,
    usableBounds: { left: 0, top: 0, right: 1, bottom: 1 },
  }));
  const cloudLayerRef = useRef<CloudLayerState>({
    quality: "disabled",
    cloudStatus: "idle",
    systems: [],
    drift: 0,
  ...
  const getGlobeGeometry = useCallback(
    (
      dimensions: { width: number; height: number },
      zoom: number,
      usableBounds?: GlobeUsableBounds,
  ...
  useEffect(() => {
    getGlobeGeometryRef.current = getGlobeGeometry;
  }, [getGlobeGeometry]);
  ...
  const getInteractionEngine = useCallback(() => {
    if (!interactionEngineRef.current) {
      interactionEngineRef.current = new AtlasInteractionEngine({
        ...
        countryResolver: (point) =>
          countryAtPoint(point.lat, point.lng, runtimeRef.current.landShapes),
        geometryProvider: () => {
          ...
          const geometry = getGlobeGeometryRef.current(
            { width: rect.width, height: rect.height },
            s.zoom,
            viewportDebug?.usableBounds,
          );
          return {
            geometry: geometry as AtlasInteractionGeometry,
            rotation: { rotX: s.rotX, rotY: s.rotY },
          };
        },
      });
    }
  ...
  const focusPoint = useCallback(
    (point: GlobePoint | null, fast = false) => {
      if (!point) return;
      const focusKey = stationFocusIdentityKey;
      const s = state.current;
      if (lastFocusKeyRef.current === focusKey && !s.disabledMotion) {
        debugGlobeFocus("focusPoint duplicate restarting", {
          selectionVersion,
          focusKey,
          label: point.label,
          fast,
          reason:
            "same station/point/version requested again; recentering to prevent drift",
      ...
      const previousPoint =
        previousFocusRef.current?.point ??
        (previousStation ? stationPoint(previousStation) : null);
      const rotation = focusRotationForPoint(point);
      const targetRotX = clampFocusLatitude(rotation.rotX);
      const shortestDeltaY = Math.atan2(
        Math.sin(rotation.rotY - s.rotY),
        Math.cos(rotation.rotY - s.rotY),
      );
      const targetRotY = s.rotY + shortestDeltaY;
      const rotationAngularDistance = Math.min(
        Math.PI,
        Math.abs(shortestDeltaY) + Math.abs(targetRotX - s.rotX) * 0.7,
      );
      const stationAngularDistance = angularDistanceBetweenPoints(
        previousPoint,
        point,
      );
      ...
      const focusDuration = resolveFocusDuration({
        disabledMotion: s.disabledMotion,
        fast,
        mobile,
        angularDistance,
        iosWebKit,
      ...
      if (
        ...
        focusDuration <
          (iosWebKit
            ? IOS_MINIMUM_FOCUS_DURATION_MS
            : MINIMUM_FOCUS_DURATION_MS)
      )
        warnGlobeFocus("focusDuration below minimum", {
          selectionVersion,
          label: point.label,
          focusDuration,
          minimumFocusDuration: iosWebKit
            ? IOS_MINIMUM_FOCUS_DURATION_MS
            : MINIMUM_FOCUS_DURATION_MS,
          reducedMotion: s.disabledMotion,
          fast,
          fastFlagSource: "BlueMarbleGlobe.teleporting prop",
          mobile,
          iosWebKit,
        });
      debugGlobeFocus("focusRotationForPoint", {
        selectionVersion,
        label: point.label,
        latitude: point.lat,
        longitude: point.lng,
        rotX: rotation.rotX / DEG,
        rotY: rotation.rotY / DEG,
      });
      s.focusToken += 1;
      ...
      wrapRef.current?.setAttribute(
        "data-globe-travel-active",
        String(!s.disabledMotion),
      );
      window.dispatchEvent(
        new CustomEvent("waveatlas:globe-travel", {
          detail: {
            active: !s.disabledMotion,
            iosWebKit,
            startedAt: Date.now(),
            selectionVersion,
          },
      ...
      debugGlobeFocus("focusPoint", {
        selectionVersion,
        fastFlagSource: "BlueMarbleGlobe.teleporting prop",
        minimumFocusDuration: MINIMUM_FOCUS_DURATION_MS,
        primaryEasing: "easeInOutCubic",
        arrivalEasing: "easeOutQuart",
        previousStation:
          previousFocusRef.current?.name ?? previousStation?.name ?? null,
        previousLat: previousPoint?.lat ?? null,
        previousLng: previousPoint?.lng ?? null,
        previousRotX: s.rotX / DEG,
        previousRotY: s.rotY / DEG,
        previousZoom: s.zoom,
        nextStation: station.name,
        nextLat: point.lat,
        nextLng: point.lng,
        startRotation: {
          rotX: s.travelStartX / DEG,
          rotY: s.travelStartY / DEG,
        },
        targetRotation: { rotX: targetRotX / DEG, rotY: targetRotY / DEG },
        computedTargetRotX: targetRotX / DEG,
        computedTargetRotY: targetRotY / DEG,
        targetZoom,
        shortestDeltaY: shortestDeltaY / DEG,
        angularDistance: angularDistance / DEG,
        stationAngularDistance:
          stationAngularDistance === null ? null : stationAngularDistance / DEG,
        rotationAngularDistance: rotationAngularDistance / DEG,
        iosWebKit,
        browser: describeBrowser(),
        focusDuration,
        durationNearZero: focusDuration <= 16,
        mobile,
        reducedMotion: s.disabledMotion,
        fast,
        correctionPass: false,
        geoSource: point.geoSource,
        geoPrecision: point.geoPrecision,
        stages: [
          "zoom-out",
          "shortest-path-rotation",
          "final-centered-approach",
          "landing-pulse",
        ],
      });
      ...
      if (s.disabledMotion) {
        debugGlobeFocus("focusPoint reduced-motion snap", {
          selectionVersion,
          label: point.label,
          targetRotX: s.targetX / DEG,
          targetRotY: s.targetY / DEG,
  ...
  useEffect(() => {
    streetZoomRequestRef.current = onStreetZoomRequest;
    streetZoomTriggeredRef.current = false;
  }, [onStreetZoomRequest, station.id]);
  const requestStreetZoom = useCallback(
    (zoom: number, reason: string) => {
      ...
      if (process.env.NODE_ENV !== "production")
        console.info("[WaveAtlas] atlas view transition", {
          fromView: "globe",
          toView: "map",
  ...
  useEffect(() => {
    ...
    debugGlobeDecision({
      ...
      visualViewport: window.visualViewport
        ...
        : null,
      device: profile.mobile ? "mobile" : "desktop",
      browser: describeBrowser(),
      iosWebKit,
      userAgent: navigator.userAgent,
      lowPower: profile.lowPower,
      webglSupport: profile.webgl,
    ...
    try {
      ...
      const reason =
        error instanceof Error
          ? error.message
          : "Canvas globe initialization failed.";
      console.warn("[WaveAtlas globe] canvas initialization failed", {
        reason,
        timestamp: Date.now(),
      });
    ...
    if (!ctx) {
      console.warn("[WaveAtlas globe] canvas initialization failed", {
        reason: "Canvas globe context unavailable.",
        timestamp: Date.now(),
      });
    ...
    debugGlobeFocus("reducedMotion detection", {
      reducedMotion: state.current.disabledMotion,
    ...
    const transitionStats = {
      ...
      maxFrameGap: 0,
      totalFrameGap: 0,
      startSize: null as CanvasSize | null,
      canvasSizeChanged: false,
      resizeEvents: [] as string[],
    };
    const fallbackTimer = mobile
      ? window.setTimeout(() => {
          if (!painted)
            fallbackRef.current?.(
              "Globe view is optimized for this device using map mode.",
            );
        }, MOBILE_FALLBACK_MS)
      : 0;
    ...
    const initializeCloudLayer = () => {
      if (cloudLayer.cloudStatus !== "idle" || !PHOTOREALISTIC_CLOUDS_ENABLED)
        return;
      cloudLayer.quality = resolveCloudQuality(
        profile,
        iosWebKit,
        state.current.disabledMotion,
      );
      cloudLayer.iosReduced = iosWebKit && cloudLayer.quality === "reduced";
      if (cloudLayer.quality === "disabled") {
        cloudLayer.cloudStatus = "failed";
        debugClouds({
          cloudStatus: "failed",
          quality: cloudLayer.quality,
          reason: "clouds disabled by capability",
        });
        return;
      }
      cloudLayer.cloudStatus = "loading";
      ...
      window.setTimeout(
        () => {
          ...
          try {
            cloudLayer.systems = buildCloudSystems(cloudLayer.quality);
            cloudLayer.cloudStatus = "ready";
            resetCloudDriftClock(cloudLayer);
            cloudLayer.performanceStartedAt = cloudLayer.lastDriftAt;
            debugClouds({
              quality: cloudLayer.quality,
              cloudStatus: cloudLayer.cloudStatus,
              iosReduced: cloudLayer.iosReduced,
              systemCount: cloudLayer.systems.length,
              driftStatus: cloudLayer.driftStatus,
            });
          } catch (error) {
            cloudLayer.quality = "disabled";
            cloudLayer.cloudStatus = "failed";
            cloudLayer.downgradeReason = "cloud system generation failed";
            debugClouds({
              cloudStatus: "failed",
    ...
    const downgradeClouds = (reason: string) => {
      if (cloudLayer.quality === "full") {
        cloudLayer.quality = "reduced";
        cloudLayer.systems = buildCloudSystems("reduced");
      } else {
        cloudLayer.quality = "disabled";
        cloudLayer.cloudStatus = "failed";
        cloudLayer.systems = [];
    ...
    const drawLabel = (
        ...
        : isCountry
          ...
          : kind === "city"
            ...
            : mobile
              ? 8
              : 9;
      const weight = active ? 800 : isCountry ? 500 : 600;
      const y =
        p.y - (active ? (mobile ? 24 : 28) : kind === "station" ? 16 : 0);
      ...
      const box = {
        left: p.x - width / 2 - padX,
        right: p.x + width / 2 + padX,
        top: y - fontSize / 2 - padY,
        bottom: y + fontSize / 2 + padY,
        active,
      };
      ...
      ctx.lineWidth = active ? 4.5 : isCountry ? 2.25 : 3;
      ...
      ctx.fillStyle = active
        ...
        : isCountry
          ...
          : kind === "city"
            ...
            : "rgba(209,250,229,0.74)";
      ctx.fillText(displayLabel, p.x, y, mobile ? 112 : 150);
      ctx.restore();
      return true;
    };

    const drawProgressiveLabels = (
      projection: D3GeoProjection,
      activeBeacon: GlobePoint | null,
      ...
      if (zoom <= 1.48) {
        ...
        for (const shape of runtime.landShapes) {
          ...
          labels.push({
            ...
            label: shape.name,
            kind: "country",
            priority: Math.max(0, 180 - Math.abs(centroid.lat)),
          });
          if (
            labels.filter((item) => item.kind === "country").length >=
            countryLimit * 2
          )
            break;
        }
      ...
      if (zoom >= 1.18 && zoom < 1.9) {
        ...
        for (const city of CITY_LIGHTS.slice(0, cityLimit + 6))
          labels.push({
            ...city,
            kind: "city",
            priority:
              120 -
              (activeBeacon ? angularDistanceDegrees(city, activeBeacon) : 0),
          });
      }
      if (activeBeacon && zoom >= 1.72) {
        ...
        const nearbyStations = runtime.signalFeatures
          ...
          .filter(
            (label) =>
              angularDistanceDegrees(label, activeBeacon) <=
              (zoom >= 1.95 ? 24 : 14),
          )
      ...
      for (const label of labels.sort(
        ...
        drawLabel(label, projection, { force: Boolean(label.active) });
    };

    const drawFrame = (now: number) => {
      const frameStartedAt = now;
      const frameMs =
        mobile || profile.lowPower ? MOBILE_FRAME_MS : DESKTOP_FRAME_MS;
      ...
      const dt =
        iosWebKit && state.current.travelActive
          ? Math.min(IOS_FRAME_DELTA_CLAMP_MS, rawDt)
          : Math.min(50, rawDt);
      then = now;
      if (state.current.travelActive) {
        if (!transitionStats.active) {
          transitionStats.active = true;
          transitionStats.startedAt = now;
          transitionStats.frameCount = 0;
          transitionStats.droppedFrames = 0;
          transitionStats.maxFrameGap = 0;
          transitionStats.totalFrameGap = 0;
          transitionStats.startSize = stableSizeRef.current;
          transitionStats.canvasSizeChanged = false;
          transitionStats.resizeEvents = [];
        }
        transitionStats.frameCount += 1;
        transitionStats.maxFrameGap = Math.max(
          transitionStats.maxFrameGap,
          rawDt,
        );
        transitionStats.totalFrameGap += rawDt;
        if (rawDt > 34)
          transitionStats.droppedFrames += Math.max(
            1,
            Math.floor(rawDt / 16.7) - 1,
          );
      }
      ...
      const dpr =
        size?.dpr ??
        Math.min(
          mobile || profile.lowPower ? 1.25 : 2,
      ...
      if (
        transitionStats.active &&
        transitionStats.startSize &&
        (transitionStats.startSize.cssWidth !== w ||
          transitionStats.startSize.cssHeight !== h ||
          transitionStats.startSize.pixelWidth !== pixelWidth ||
          transitionStats.startSize.pixelHeight !== pixelHeight)
      )
        transitionStats.canvasSizeChanged = true;
      if (canvas.width !== pixelWidth || canvas.height !== pixelHeight) {
        if (
          !(iosWebKit && state.current.travelActive && stableSizeRef.current)
        ) {
      ...
      const s = state.current;
      const focusElapsed = s.focusStartedAt
        ? now - s.focusStartedAt
        : s.focusDuration;
      const focusProgress =
        s.focusDuration <= 0 ? 1 : Math.min(1, focusElapsed / s.focusDuration);
      if (s.disabledMotion) {
        s.rotX = s.targetX;
        s.rotY = s.targetY;
        s.zoom = s.targetZoom;
        ...
        const rotateProgress = iosWebKit
          ? easeInOutCubic(focusProgress)
          : focusProgress < zoomOutEnd
            ? easeInOutCubic(focusProgress / zoomOutEnd) * 0.08
            : focusProgress < approachStart
              ? 0.08 +
                easeInOutCubic(
                  (focusProgress - zoomOutEnd) / (approachStart - zoomOutEnd),
                ) *
                  0.76
              : 0.84 +
                easeOutQuart(
                  (focusProgress - approachStart) / (1 - approachStart),
                ) *
                  0.16;
        const zoomOut = Math.max(
          0.86,
          s.travelStartZoom - (mobile ? 0.1 : 0.14),
        );
        const stagedZoom =
          focusProgress < approachStart
            ? lerp(
                s.travelStartZoom,
                zoomOut,
                easeOutQuart(Math.min(1, focusProgress / approachStart)),
              )
            : lerp(
                zoomOut,
                s.targetZoom,
        ...
        if (s.travelActive) {
          ...
          s.landingPulseStartedAt = now;
        }
      }
      const activeFocusVerified = runtime.currentPoint
        ? s.verifiedPointKey === runtime.focusIdentityKey
        : true;
      if (
        s.activeFocusIdentityKey !== runtime.focusIdentityKey &&
        s.travelActive
      ) {
        debugGlobeFocus("stale transition cancelled", {
          activeFocusIdentityKey: s.activeFocusIdentityKey,
          runtimeFocusIdentityKey: runtime.focusIdentityKey,
          activeSelectionVersion: s.activeSelectionVersion,
          runtimeSelectionVersion: runtime.selectionVersion,
          travelActive: s.travelActive,
        });
        s.travelActive = false;
      }
      if (
        globeDebugEnabled() &&
        s.focusStartedAt &&
        s.activeSelectionVersion === runtime.selectionVersion
      ) {
        for (const milestone of [0, 25, 50, 75, 100]) {
          if (
            focusProgress >= milestone / 100 &&
            !s.progressMilestones.has(milestone)
          ) {
            s.progressMilestones.add(milestone);
            debugGlobeFocus("animation progress", {
              selectionVersion: runtime.selectionVersion,
              station: runtime.stationName,
      ...
      if (
        ...
        s.targetY +=
          (mobile ? 0.00016 : 0.00035) *
          (runtime.teleporting ? (mobile ? 1.4 : 2.6) : 1);
      const viewportDebug =
        mobile || globeDebugEnabled() ? readMobileViewport(wrap, canvas) : null;
      const geometry = getGlobeGeometry(
        { width: w, height: h },
        s.zoom,
        viewportDebug?.usableBounds,
      );
      ...
      bg.addColorStop(0.64, "rgba(3,12,27,0.10)");
      bg.addColorStop(1, "rgba(3,8,20,0)");
      ctx.fillStyle = bg;
      ...
      if (runtime.basemap === "night") {
        ocean.addColorStop(0, "#111827");
        ocean.addColorStop(0.55, "#050816");
        ocean.addColorStop(1, "#01030a");
      } else if (runtime.basemap === "signal") {
        ocean.addColorStop(0, "#08213a");
        ocean.addColorStop(0.55, "#031225");
        ocean.addColorStop(1, "#010814");
      } else {
        ocean.addColorStop(0, "#1f6f9d");
        ocean.addColorStop(0.42, "#0c3b67");
        ocean.addColorStop(1, "#031327");
      }
      const projection = buildGlobeProjection(
        w,
        h,
        r,
        state.current.rotX,
      ...
      if (!photorealisticPreview && runtime.landShapes.length) {
        ...
        ctx.fillStyle = photorealisticPreview
          ? "rgba(57,118,74,0.88)"
          : runtime.basemap === "night"
            ? "rgba(30,41,59,0.78)"
            : runtime.basemap === "signal"
      ...
      if (photorealisticPreview) {
        ...
        for (const light of lights) {
          const p = project(light.lat, light.lng, projection);
          if (p.z < -0.02) continue;
          const nightBoost = Math.max(0.15, 1 - Math.max(0, p.z) * 0.65);
          const glow = (1 + Math.max(0, p.z) * 1.2) * nightBoost;
          ctx.fillStyle = "rgba(251,191,36,0.24)";
          ctx.beginPath();
          ctx.arc(p.x, p.y, 4.8 * glow, 0, TAU);
          ctx.fill();
          ctx.fillStyle = "rgba(255,244,180,0.78)";
          ctx.beginPath();
          ctx.arc(p.x, p.y, 1.05 * glow, 0, TAU);
          ctx.fill();
        }
      }
      if (runtime.basemap === "night") {
        ...
        for (const light of lights) {
          const p = project(light.lat, light.lng, projection);
          if (p.z < -0.02) continue;
          const glow = 1 + Math.max(0, p.z) * 1.8;
          ctx.fillStyle = "rgba(251,191,36,0.24)";
          ctx.beginPath();
          ctx.arc(p.x, p.y, 5.5 * glow, 0, TAU);
          ctx.fill();
          ctx.fillStyle = "rgba(255,244,180,0.88)";
          ctx.beginPath();
          ctx.arc(p.x, p.y, 1.4 * glow, 0, TAU);
          ctx.fill();
        }
      }
      ctx.restore();
      if (photorealisticPreview) {
        ...
        ctx.beginPath();
        ctx.arc(cx, cy, r + (mobile ? 3 : 5), 0, TAU);
        ctx.stroke();
      ...
      if (!s.travelActive && signalRefreshKeyRef.current !== liveSignalKey) {
        ...
        const maxSignals =
          liveSignalBucket === "world"
            ? 0
            : mobile
              ? liveSignalBucket === "city"
                ? 250
                : 110
              : liveSignalBucket === "city"
                ? 640
                : 220;
        const nextSignals = buildSignalFeatures({
          stations,
          currentStation: station,
          globeVisibleHemisphere: liveCenter,
          globeScale: s.zoom,
          favoriteIds: readGlobeFavoriteSet(),
          maxSignals,
          debug: DEBUG_SIGNALS,
        ...
        if (DEBUG_SIGNALS || process.env.NODE_ENV !== "production")
          console.debug("[WaveAtlas signals] refresh", {
            view: "globe",
            activeStation: station.name,
            zoomLevel: s.zoom,
            bucket: liveSignalBucket,
            renderedSignals: nextSignals.visibleSignals.length,
            renderedClusters: nextSignals.clusters.length,
      ...
      if (runtime.signalClusters.length && s.zoom >= 1.16 && s.zoom < 1.9) {
        for (const cluster of runtime.signalClusters.slice(
          ...
          const radius = Math.min(
            mobile ? 15 : 20,
            5 + Math.sqrt(cluster.count) * (mobile ? 1.45 : 2.05),
          );
          ctx.fillStyle = "rgba(0,214,143,0.23)";
          ctx.beginPath();
          ctx.arc(p.x, p.y, radius * 2.15, 0, TAU);
          ctx.fill();
          ctx.fillStyle = "rgba(0,214,143,0.84)";
          ctx.beginPath();
      ...
      if (runtime.signalFeatures.length && s.zoom >= 1.45) {
        ...
        for (const feature of runtime.signalFeatures.slice(
          0,
          majorOnly ? (mobile ? 36 : 72) : runtime.signalFeatures.length,
        )) {
          ...
          if (favorite) {
            ...
            ctx.lineWidth = 1.7;
            ctx.beginPath();
            ctx.arc(p.x, p.y, mobile ? 5.8 : 7.8, 0, TAU);
            ctx.stroke();
          }
          ctx.fillStyle =
            feature.properties.status === "community"
              ? "#48C7FF"
              : feature.properties.status === "unverified"
                ? "#D4A64A"
                : "#00D68F";
          ctx.globalAlpha = majorOnly ? 0.8 : 0.9;
          ctx.beginPath();
          ctx.arc(
            p.x,
            p.y,
            majorOnly ? (mobile ? 2.3 : 3.1) : mobile ? 1.95 : 2.8,
            0,
            TAU,
          );
          ctx.fill();
      ...
      if (activeBeacon) {
        ...
        const withinTarget =
          ...
          Number.isFinite(p.y) &&
          frontFacing &&
          p.x >= usableBounds.left &&
          p.x <= usableBounds.right &&
          p.y >= usableBounds.top &&
          p.y <= usableBounds.bottom &&
          Math.hypot(deltaX, deltaY) <= (mobile ? 36 : 54);
        ...
        if (focusProgress >= 1 && s.verificationPending && !s.dragging) {
          if (!withinTarget && !s.correctiveFocusRan) {
            const frontCorrectionY = frontFacing
              ? 0
              : Math.atan2(
                  Math.sin(
                    focusRotationForPoint(activeBeacon).rotY - s.targetY,
                  ),
                  Math.cos(
                    focusRotationForPoint(activeBeacon).rotY - s.targetY,
                  ),
                );
            const horizontalCorrection = Number.isFinite(deltaX)
              ? deltaX / Math.max(1, r * Math.max(0.35, Math.abs(p.z)))
              : 0;
            const verticalCorrection = Number.isFinite(deltaY)
              ? -deltaY / Math.max(1, r)
              : 0;
            s.targetY += frontCorrectionY + horizontalCorrection;
            s.targetX = Math.max(
              -70 * DEG,
              Math.min(70 * DEG, s.targetX + verticalCorrection),
            );
            ...
            s.travelStartZoom = s.zoom;
            s.travelActive = !s.disabledMotion;
            s.focusDuration = s.disabledMotion
              ? 0
              : iosWebKit
                ? IOS_POST_FOCUS_CORRECTION_DURATION_MS
                : POST_FOCUS_CORRECTION_DURATION_MS;
            s.correctiveFocusRan = true;
            debugGlobeFocus("post-focus corrective pass", {
              snapDirectly: s.disabledMotion,
              correctionMode: s.disabledMotion ? "instant" : "animated",
              correctionDuration: s.focusDuration,
              correctionPass: true,
              station: runtime.stationName,
              city: runtime.stationCity,
              country: runtime.stationCountry,
              selectionVersion: runtime.selectionVersion,
              stationLat: activeBeacon.lat,
              stationLng: activeBeacon.lng,
              derivedGlobePoint: activeBeacon,
              targetRotation: { rotX: s.targetX / DEG, rotY: s.targetY / DEG },
              actualProjectedX: p.x,
              actualProjectedY: p.y,
              targetX: targetScreenX,
              targetY: targetScreenY,
              deltaX,
              deltaY,
              frontFacing,
              correctiveFocusRan: true,
              usableBounds,
            });
          } else {
            s.verificationPending = false;
            s.verifiedPointKey = pointKey;
            debugGlobeFocus("post-focus verified", {
              snapDirectly: false,
              station: runtime.stationName,
              city: runtime.stationCity,
              country: runtime.stationCountry,
              ...
              deltaY,
              frontFacing,
              correctiveFocusRan: s.correctiveFocusRan,
              usableBounds,
            });
          }
        }
        if (globeDebugEnabled() && now - debugOverlayTickRef.current > 250) {
          ...
          setDebugOverlay({
            ...
            rotX: s.rotX / DEG,
            rotY: s.rotY / DEG,
            selectedCountry: country?.name ?? null,
            frontFacing,
            correctiveFocusRan: s.correctiveFocusRan,
            d3InputOrder: "[longitude, latitude]",
          });
        ...
        if (lastCoordinateLogRef.current !== logKey) {
          ...
          debugGlobeCoordinates({
            station: runtime.stationName,
            selectionVersion: runtime.selectionVersion,
            label: runtime.stationLabel,
            latitude: activeBeacon.lat,
            longitude: activeBeacon.lng,
            d3ProjectionInput: [activeBeacon.lng, activeBeacon.lat],
            d3ProjectionInputOrder: "[longitude, latitude]",
            rotation: {
              rotX: s.rotX / DEG,
              rotY: s.rotY / DEG,
              d3Rotate: projection.rotate(),
            },
            computed3DVector: p.vector,
            renderedMarkerPosition: { x: p.x, y: p.y, z: p.z },
            beaconScreenX: p.x,
            beaconScreenY: p.y,
            targetScreenX,
            targetScreenY,
        ...
        if (focusProgress >= 1 && settledFocusLogRef.current !== settledKey) {
          ...
          debugGlobeFocus("animation settled projection", {
            station: runtime.stationName,
            selectionVersion: runtime.selectionVersion,
            label: runtime.stationLabel,
            beaconScreenX: p.x,
            ...
            canvasHeight: h,
            devicePixelRatio: dpr,
            visualViewportHeight: viewportDebug?.visualViewport
              ? viewportDebug.visualViewport.height
              : null,
            safeAreaBottomEstimate:
              viewportDebug?.safeAreaBottomEstimate ?? null,
            dockRect: viewportDebug?.dockRect ?? null,
            playerRect: viewportDebug?.playerRect ?? null,
            usableBounds,
          });
        }
        if (p.z > -0.05) {
          const approachVisibility = s.disabledMotion
            ? 1
            : Math.min(1, Math.max(0.18, (focusProgress - 0.5) / 0.42));
          drawActiveStationBeacon(ctx, p.x, p.y, {
            now,
            mobile,
            lowPower: profile.lowPower,
            reducedMotion: s.disabledMotion,
            visibility: approachVisibility,
            landingPulseStartedAt: s.landingPulseStartedAt,
          });
          if (approachVisibility > 0.72 && !(iosWebKit && s.travelActive))
            drawProgressiveLabels(projection, activeBeacon, runtime, s.zoom);
        }
      }
      ctx.restore();
      if (globeDebugEnabled() && activeBeacon) {
        ...
        ctx.strokeRect(
          usableBounds.left,
          usableBounds.top,
          usableBounds.right - usableBounds.left,
          usableBounds.bottom - usableBounds.top,
        );
        ctx.setLineDash([]);
        ...
        ctx.beginPath();
        ctx.moveTo(targetScreenX - 16, targetScreenY);
        ctx.lineTo(targetScreenX + 16, targetScreenY);
        ctx.moveTo(targetScreenX, targetScreenY - 16);
        ctx.lineTo(targetScreenX, targetScreenY + 16);
        ctx.stroke();
        ctx.fillStyle = "rgba(255,56,56,0.95)";
        ctx.beginPath();
        ctx.arc(projected.x, projected.y, 4.5, 0, TAU);
        ctx.fill();
      ...
      if (
        ...
        if (cloudLayer.frameCount >= CLOUD_PERFORMANCE_WINDOW_FRAMES) {
          ...
          const threshold =
            mobile || profile.lowPower || iosWebKit
              ? CLOUD_MOBILE_FRAME_THRESHOLD_MS
              : CLOUD_DESKTOP_FRAME_THRESHOLD_MS;
          cloudLayer.slowWindows =
            avgFrameMs > threshold ? cloudLayer.slowWindows + 1 : 0;
          if (cloudLayer.slowWindows >= CLOUD_SLOW_WINDOWS_BEFORE_DOWNGRADE)
            downgradeClouds(
              `sustained total frame cost ${avgFrameMs.toFixed(1)}ms`,
            );
          else {
            cloudLayer.frameCount = 0;
            cloudLayer.totalFrameMs = 0;
      ...
      if (transitionStats.active && !s.travelActive) {
        const avgGap = transitionStats.frameCount
          ? transitionStats.totalFrameGap / transitionStats.frameCount
          : 0;
        const projected = activeBeacon
          ? projectGlobePoint(
              activeBeacon,
              { rotX: s.rotX, rotY: s.rotY },
              geometry,
          ...
          : null;
        debugGlobeFocus("transition ended", {
          selectionVersion: runtime.selectionVersion,
          iosWebKit,
          endedAt: Date.now(),
          durationMs: now - transitionStats.startedAt,
          frameCount: transitionStats.frameCount,
          averageFps: avgGap ? 1000 / avgGap : null,
          maxFrameGap: transitionStats.maxFrameGap,
          droppedFrames: transitionStats.droppedFrames,
          canvasSizeChanged: transitionStats.canvasSizeChanged,
          resizeEvents: transitionStats.resizeEvents,
          finalBeaconDelta: projected
            ? { x: projected.x - targetScreenX, y: projected.y - targetScreenY }
            : null,
        });
    ...
    const draw = (now: number) => {
      try {
        ...
        const reason =
          error instanceof Error ? error.message : "Globe render loop failed.";
        if (process.env.NODE_ENV !== "production")
          console.error("[WaveAtlas globe] render error", error);
        fallbackRef.current?.(reason);
    ...
    const debouncedSyncCanvasSize = (eventName = "resize") => {
      if (state.current.travelActive) {
        transitionStats.resizeEvents.push(eventName);
        warnGlobeFocus("viewport/layout event during transition", {
          eventName,
          iosWebKit,
          visualViewport: window.visualViewport
            ? {
            ...
            : null,
          canvasSize: stableSizeRef.current,
        });
        if (iosWebKit) return;
      }
      window.clearTimeout(throttleTimer);
      throttleTimer = window.setTimeout(syncCanvasSize, mobile ? 160 : 80);
    };
    syncCanvasSize();
    const resizeObserver =
      typeof ResizeObserver !== "undefined"
        ? new ResizeObserver(() => debouncedSyncCanvasSize("ResizeObserver"))
        : null;
    resizeObserver?.observe(wrap);
    const onOrientationChange = () =>
      debouncedSyncCanvasSize("orientationchange");
    const onWindowResize = () => debouncedSyncCanvasSize("window.resize");
    const onVisualViewportResize = () =>
      debouncedSyncCanvasSize("visualViewport.resize");
    const onVisualViewportScroll = () =>
      debouncedSyncCanvasSize("visualViewport.scroll");
    ...
    const onVisibility = () => {
      state.current.hidden = document.hidden;
      resetCloudDriftClock(cloudLayer);
      cloudLayer.driftStatus = document.hidden
        ? "paused"
        : cloudLayer.driftStatus;
      if (document.hidden) {
        cancelAnimationFrame(raf);
        raf = 0;
    ...
    const onReducedMotionChange = () => {
      state.current.disabledMotion = reducedMotionQuery.matches;
      resetCloudDriftClock(cloudLayer);
      if (cloudLayer.cloudStatus === "ready") {
        cloudLayer.quality = resolveCloudQuality(
          profile,
          iosWebKit,
          state.current.disabledMotion,
    ...
    const onContextLost = () => {
      cloudLayer.contextLost = true;
      cloudLayer.quality = "disabled";
      cloudLayer.cloudStatus = "failed";
      cloudLayer.systems = [];
      cloudLayer.downgradeReason = "canvas context lost";
      debugClouds({ contextLost: true, quality: cloudLayer.quality });
    };
    document.addEventListener("visibilitychange", onVisibility);
    window.addEventListener("pageshow", onPageShow);
    reducedMotionQuery.addEventListener?.("change", onReducedMotionChange);
    const supportsCanvasContextLost = "oncontextlost" in canvas;
    if (supportsCanvasContextLost)
    ...
    return () => {
      ...
      cloudLayer.quality = "disabled";
      cloudLayer.driftStatus = "disabled";
  ...
  useEffect(
    () => focusPoint(currentPoint, teleporting),
    [currentPoint, focusPoint, stationFocusIdentityKey, teleporting],
  );

  useEffect(() => {
    stationsRef.current = stations;
    if (process.env.NODE_ENV !== "production")
      logStationGeoTruthHealthReport(stations);
  }, [stations]);
  useEffect(() => {
    activeStationKeyRef.current = station.station_uuid || station.id;
  }, [station.station_uuid, station.id]);

  const handlePointerDown = (event: React.PointerEvent<HTMLCanvasElement>) => {
    event.preventDefault();
    const s = state.current;
    debugGlobeFocus("user drag cancelled transition", {
      selectionVersion,
      station: station.name,
      travelActive: s.travelActive,
    ...
    s.downOverOverlay = isGlobePointerBlockedByOverlay(
      event.clientX,
      event.clientY,
    );
    getInteractionEngine().pointerDown({
      pointerId: event.pointerId,
      clientX: event.clientX,
      clientY: event.clientY,
      blockedByOverlay: s.downOverOverlay,
    });
  ...
  const handlePointerMove = (event: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    getInteractionEngine().pointerMove({
      pointerId: event.pointerId,
      clientX: event.clientX,
      clientY: event.clientY,
    });
    ...
    if (activePointers.length >= 2) {
      ...
      if (pinchDistance.current) {
        s.targetZoom = Math.max(
          ...
          Math.min(
            1.8,
            s.targetZoom + (distance - pinchDistance.current) * 0.003,
          ),
        );
        if (s.targetZoom >= 1.68)
          requestStreetZoom(s.targetZoom, "pinch street/city threshold");
      }
    ...
    const rotation = rotateFromDrag(
      { rotX: s.targetX, rotY: s.targetY },
      dx,
      dy,
      mobile,
    );
    s.targetX = rotation.rotX;
    s.targetY = rotation.rotY;
    s.lastX = event.clientX;
    s.lastY = event.clientY;
  };
  const handlePointerUp = (event: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    const result = getInteractionEngine().pointerUp({
      ...
      blockedByOverlay: isGlobePointerBlockedByOverlay(
        event.clientX,
        event.clientY,
      ),
    });
    debugGlobeClick({
      ...
      selectedStationDistanceKm:
        result.kind === "destination" ? result.event.distanceKm : null,
      candidateCount:
        result.kind === "destination" ? result.event.candidates.length : 0,
      zoom: s.zoom,
    });
    if (result.kind === "destination") {
      onStationSelect?.(
        result.event.station,
    ...
    if (result.kind === "country") {
      const clickLatLng =
        result.diagnostics.resolvedLatLng ?? result.country.centroid;
      onCountrySelect?.({
        ...result.country,
        ...
        source: result.diagnostics.resolvedLatLng
          ? "polygon"
          : "polygon-fallback",
      });
    }
  ...
  const handleWheel = (event: React.WheelEvent<HTMLCanvasElement>) => {
    ...
    s.targetZoom = Math.max(
      0.82,
      Math.min(1.8, s.targetZoom - event.deltaY * 0.001),
    );
    if (s.targetZoom >= 1.68)
      requestStreetZoom(s.targetZoom, "wheel street/city threshold");
  };
    ...
    >
      ...
      {globeDebugEnabled() && debugOverlay ? (
        <div className="pointer-events-none absolute bottom-4 left-4 z-30 rounded-2xl border border-emerald-300/30 bg-slate-950/80 p-3 font-mono text-[10px] leading-5 text-emerald-100 shadow-2xl backdrop-blur-xl">
          <div>
            station lat/lng: {debugOverlay.stationLat?.toFixed(4)},{" "}
            {debugOverlay.stationLng?.toFixed(4)}
          </div>
          <div>
            screen x/y: {debugOverlay.screenX?.toFixed(1)},{" "}
            {debugOverlay.screenY?.toFixed(1)}
          </div>
          <div>
            target x/y: {debugOverlay.targetScreenX.toFixed(1)},{" "}
            {debugOverlay.targetScreenY.toFixed(1)}
          </div>
          <div>
            delta x/y: {debugOverlay.deltaX?.toFixed(1)},{" "}
            {debugOverlay.deltaY?.toFixed(1)}
          </div>
          <div>
            usable: {Math.round(debugOverlay.usableBounds.left)},
            {Math.round(debugOverlay.usableBounds.top)} →{" "}
            {Math.round(debugOverlay.usableBounds.right)},
            {Math.round(debugOverlay.usableBounds.bottom)}
          </div>
          <div>
            rotX/rotY: {debugOverlay.rotX.toFixed(2)}°,{" "}
            {debugOverlay.rotY.toFixed(2)}°
          </div>
