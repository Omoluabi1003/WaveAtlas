...
export default function BlueMarbleGlobe({ station, stations = [], previousStation, teleporting = false, onCountrySelect, onStationSelect, onFallback, onStreetZoomRequest, mobile = false, basemap = "photorealistic", selectionVersion }: Props) {
  ...
  useEffect(() => {
    ...
    const drawLabel = (label: GlobeLabel, projection: D3GeoProjection, options: { force?: boolean } = {}) => {
      ...
      const fontSize = active ? (mobile ? 12 : 13) : isCountry ? (mobile ? 7.25 : 8.25) : kind === "city" ? (mobile ? 8 : 9) : (mobile ? 8 : 9);
      ...
      (ctx as CanvasRenderingContext2D & { letterSpacing?: string }).letterSpacing = isCountry ? (mobile ? "0.12em" : "0.16em") : "0px";
      ...
      ctx.strokeStyle = active ? "rgba(2,6,23,0.92)" : isCountry ? "rgba(2,6,23,0.62)" : "rgba(2,6,23,0.78)";
      ctx.strokeText(displayLabel, p.x, y, mobile ? 112 : 150);
      ctx.fillStyle = active ? "rgba(255,255,255,0.98)" : isCountry ? "rgba(248,250,252,0.68)" : kind === "city" ? "rgba(186,230,253,0.68)" : "rgba(209,250,229,0.74)";
    ...
    const drawFrame = (now: number) => {
      ...
      if (!photorealisticPreview && runtime.landShapes.length) {
        ...
        ctx.fillStyle = photorealisticPreview ? "rgba(57,118,74,0.88)" : runtime.basemap === "night" ? "rgba(30,41,59,0.78)" : runtime.basemap === "signal" ? "rgba(18,52,70,0.66)" : "rgba(42,92,78,0.82)";
        ...
        ctx.strokeStyle = photorealisticPreview ? "rgba(234,244,255,0.28)" : runtime.basemap === "night" ? "rgba(125,211,252,0.25)" : runtime.basemap === "signal" ? "rgba(125,211,252,0.38)" : "rgba(125,211,252,0.30)";
