    ...
    const drawFrame = (now: number) => {
      ...
      if (activeBeacon) {
        ...
        if (p.z > -0.05) {
          ...
          const landingPulseAge = s.landingPulseStartedAt ? now - s.landingPulseStartedAt : 9999;
          const landingBoost = landingPulseAge < 520 ? 1 + (1 - landingPulseAge / 520) * 0.42 : 1;
          const pulse = s.disabledMotion ? 1 : (1 + Math.sin(now / (mobile ? 360 : 180)) * (mobile ? 0.08 : 0.22)) * landingBoost;
          ctx.save();
          ctx.globalAlpha = approachVisibility;
          ctx.shadowColor = "rgba(255,59,48,0.58)";
          ctx.shadowBlur = mobile || profile.lowPower ? 18 : 30;
          ctx.fillStyle = mobile || profile.lowPower ? "rgba(229,57,53,0.14)" : "rgba(229,57,53,0.20)";
          ctx.beginPath(); ctx.arc(p.x, p.y, (mobile ? 13 : 18) * pulse, 0, TAU); ctx.fill();
          ctx.shadowBlur = mobile || profile.lowPower ? 24 : 42;
          ctx.strokeStyle = "rgba(255,255,255,0.58)"; ctx.lineWidth = mobile ? 1.1 : 1.4;
          ctx.beginPath(); ctx.arc(p.x, p.y, (mobile ? 17 : 23) * pulse, 0, TAU); ctx.stroke();
          ctx.strokeStyle = "rgba(214,168,79,0.42)"; ctx.lineWidth = mobile ? 1 : 1.2;
          ctx.beginPath(); ctx.arc(p.x, p.y, (mobile ? 22 : 30) * (0.82 + (pulse - 1) * 0.7), 0, TAU); ctx.stroke();
