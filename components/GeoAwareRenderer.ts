import { geoPath, type GeoProjection as D3GeoProjection } from "d3-geo";

import type { GlobeBasemapKey } from "@/components/BlueMarbleGlobe";

type RendererLandShape = { feature: unknown };
...
export function geoAwareRendererEnabled() {
  return process.env.NEXT_PUBLIC_GEOAWARE_RENDERER === "true";
}
...
export function drawGeoAwareEarthRenderer(options: GeoAwareRendererOptions) {
  ...
  const cloudShift = reducedMotion ? 0 : ((now * (performanceMode ? 0.0012 : 0.0022)) % (r * 2.4));
  ...
  for (const shape of landShapes) {
    ctx.beginPath();
    path(shape.feature as Parameters<typeof path>[0]);
    ctx.fill("evenodd");
  }
  ...
  ctx.shadowBlur = performanceMode ? 14 : 24;
  ctx.beginPath();
