      ...
      {open ? (
        ...
        <motion.section ref={sheetRef} initial={{ opacity: 0, y: 26, rotateX: -2 }} animate={{ opacity: 1, y: 0, rotateX: 0 }} exit={{ opacity: 0, y: 26 }} transition={{ duration: 0.3, ease: "easeOut" }} className="pointer-events-auto relative z-[1000] mx-auto min-h-[100dvh] w-full max-w-full overflow-x-hidden rounded-none border border-[rgba(60,45,25,0.20)] bg-[#F4EFE2] text-[#151515] shadow-[0_24px_80px_rgba(0,0,0,0.35)] [box-sizing:border-box] before:pointer-events-none before:absolute before:inset-x-6 before:top-0 before:h-8 before:rounded-b-[50%] before:bg-[linear-gradient(to_bottom,rgba(255,255,255,.65),rgba(20,20,20,.05),transparent)] sm:mb-24 sm:min-h-0 sm:w-[min(760px,calc(100vw-32px))] sm:rounded-t-[1.25rem] md:mb-28 md:w-[min(860px,calc(100vw-4rem))] md:rounded-[1.35rem]" role="dialog" aria-modal="true" aria-label="WaveAtlas Daily">
          ...
          <div className="relative max-w-full overflow-x-hidden p-4 [box-sizing:border-box] md:p-7">
            ...
            <div className="grid max-w-full gap-x-6 gap-y-5 overflow-x-hidden md:grid-cols-[minmax(0,1.15fr)_minmax(0,.85fr)]">{visibleHeadlines.map((headline, index) => <NewspaperHeadline key={`${headline.title}-${headline.url}`} headline={headline} lead={index === 0 && tab === "Front Page"} />)}</div>
