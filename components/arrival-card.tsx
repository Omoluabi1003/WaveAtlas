...
import { Globe2, PlaneLanding, Stamp } from "lucide-react";
import type { ArrivalDestination } from "@/lib/discovery/arrival-engine";
import { flagFor } from "@/lib/stations";
export function ArrivalCard({ arrival, onComplete }: { arrival?: ArrivalDestination; onComplete?: () => void }) {
  if (!arrival) return null;
  return <motion.div className="fixed inset-0 z-[90] grid place-items-center bg-midnight/85 px-5 backdrop-blur-2xl" initial={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.7 }}>
    <motion.div initial={{ y: 28, scale: 0.96, opacity: 0 }} animate={{ y: 0, scale: 1, opacity: 1 }} exit={{ y: -18, opacity: 0 }} transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] }} onAnimationComplete={onComplete} className="relative w-full max-w-xl overflow-hidden rounded-[2rem] border border-white/15 bg-slate-950/90 p-6 shadow-[0_30px_120px_rgba(0,0,0,.55)]">
      ...
      <motion.div initial={{ rotate: -16, scale: 1.4, opacity: 0 }} animate={{ rotate: -8, scale: 1, opacity: 0.9 }} transition={{ delay: 1, duration: 0.45, ease: "backOut" }} className="absolute bottom-5 right-6 rounded-full border-2 border-radio/70 px-5 py-3 font-mono text-xs font-black uppercase tracking-[.28em] text-radio"><Stamp className="mr-2 inline size-4" />Cleared</motion.div><Globe2 className="absolute bottom-7 left-7 size-7 text-sky/50" />
    </motion.div>
  </motion.div>;
}
