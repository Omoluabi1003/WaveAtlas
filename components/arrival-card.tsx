...
export function ArrivalCard({ arrival, onEnter }: { arrival?: ArrivalDestination; onEnter?: () => void }) {
  ...
  return <motion.div className="fixed inset-0 z-[90] grid place-items-center bg-midnight/85 px-5 backdrop-blur-2xl" initial={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.5 }}>
    <motion.div initial={{ y: 28, scale: 0.96, opacity: 0 }} animate={{ y: 0, scale: 1, opacity: 1 }} exit={{ y: -18, opacity: 0 }} transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] }} className="relative w-full max-w-xl overflow-visible rounded-[2rem] border border-white/15 bg-slate-950/90 p-6 shadow-[0_30px_120px_rgba(0,0,0,.55)]">
      ...
      <motion.div initial={{ rotate: -16, scale: 1.4, opacity: 0 }} animate={{ rotate: -8, scale: 1, opacity: 0.75 }} transition={{ delay: 1, duration: 0.45, ease: "backOut" }} className="pointer-events-none relative z-0 mt-6 hidden w-fit rounded-full border-2 border-radio/60 px-4 py-2 font-mono text-[10px] font-black uppercase tracking-[.28em] text-radio/80 sm:block"><Stamp className="mr-2 inline size-4" />Cleared</motion.div>
      <div className="relative z-10 mt-7 flex flex-col items-stretch justify-between gap-4 pb-5 sm:flex-row sm:flex-wrap sm:items-center sm:gap-6">
        ...
        <button type="button" onClick={onEnter} className="relative z-20 inline-flex min-h-12 shrink-0 items-center justify-center gap-2 rounded-full bg-radio px-5 py-3 text-sm font-black text-midnight shadow-[0_0_40px_rgba(88,225,132,.24)] transition hover:scale-[1.02]">Board Flight <ArrowRight className="size-4" /></button>
      </div>
    </motion.div>
  </motion.div>;
}
