  ...
  return <motion.div className="fixed inset-0 z-[90] grid place-items-center bg-midnight/85 px-5 backdrop-blur-2xl" initial={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.7 }}>
    <motion.div initial={{ y: 28, scale: 0.96, opacity: 0 }} animate={{ y: 0, scale: 1, opacity: 1 }} exit={{ y: -18, opacity: 0 }} transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] }} className="relative w-full max-w-xl overflow-hidden rounded-[2rem] border border-white/15 bg-slate-950/90 p-6 shadow-[0_30px_120px_rgba(0,0,0,.55)]">
      ...
      <div className="relative mt-7 flex flex-wrap items-center justify-between gap-4">
        <p className="max-w-xs text-xs text-ivory/55">Preparing your first signal now. Continue immediately or enjoy the passport stamp.</p>
        <button type="button" onClick={onEnter} className="inline-flex items-center gap-2 rounded-full bg-radio px-5 py-3 text-sm font-black text-midnight shadow-[0_0_40px_rgba(88,225,132,.24)] transition hover:scale-[1.02]">Enter WaveAtlas <ArrowRight className="size-4" /></button>
      </div>
