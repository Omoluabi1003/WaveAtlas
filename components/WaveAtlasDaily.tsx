...
export function WaveAtlasDaily({ station, open, onClose }: { station: Station; open: boolean; onClose: () => void }) {
  ...
  return (
    <AnimatePresence>
      {open ? (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="pointer-events-auto md:pointer-events-none fixed inset-0 z-[65] h-[100dvh] w-screen max-w-[100vw] overflow-x-hidden overflow-y-auto bg-black/20 px-4 pb-[max(32px,env(safe-area-inset-bottom))] pt-[max(20px,env(safe-area-inset-top))] backdrop-blur-[2px]" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
