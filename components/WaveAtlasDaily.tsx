...
export function WaveAtlasDaily({ station, open, onClose }: { station: Station; open: boolean; onClose: () => void }) {
  ...
  return (
    <AnimatePresence>
      {open ? (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="pointer-events-none fixed inset-0 z-[65] h-[100dvh] w-screen max-w-[100vw] overflow-x-hidden bg-black/10 px-4 pb-[max(32px,env(safe-area-inset-bottom))] pt-[max(20px,env(safe-area-inset-top))] md:bg-transparent md:p-0" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
        <motion.section ref={sheetRef} initial={{ opacity: 0, y: 26, x: 24, rotateX: -2 }} animate={{ opacity: 1, y: 0, x: 0, rotateX: 0 }} exit={{ opacity: 0, y: 26, x: 24 }} transition={{ duration: 0.3, ease: "easeOut" }} className="pointer-events-auto fixed inset-x-4 bottom-0 z-[65] max-h-[70dvh] w-auto max-w-full overflow-y-auto overflow-x-hidden rounded-t-[1.25rem] border border-[rgba(60,45,25,0.20)] bg-[#F4EFE2] text-[#151515] shadow-[0_24px_80px_rgba(0,0,0,0.35)] [box-sizing:border-box] before:pointer-events-none before:absolute before:inset-x-6 before:top-0 before:h-8 before:rounded-b-[50%] before:bg-[linear-gradient(to_bottom,rgba(255,255,255,.65),rgba(20,20,20,.05),transparent)] md:bottom-28 md:left-auto md:right-6 md:top-28 md:max-h-none md:w-[min(420px,calc(100vw-48px))] md:rounded-[1.35rem] md:border-white/15 md:bg-[rgba(7,17,31,0.72)] md:text-ivory md:shadow-2xl md:shadow-black/35 md:backdrop-blur-2xl xl:right-8" role="dialog" aria-modal="true" aria-label="WaveAtlas Daily">
          ...
          <div className="relative max-w-full overflow-x-hidden p-4 [box-sizing:border-box] md:p-7">
            ...
            <header className="max-w-full overflow-x-hidden border-b-4 border-double border-[#151515] pb-3 text-center">
              ...
              <h2 className="mt-1 max-w-full font-serif text-5xl font-black leading-none tracking-[-0.07em] text-[#151515] [hyphens:auto] [overflow-wrap:anywhere] [word-break:break-word] md:text-4xl">{editionTitle}</h2>
            ...
            <div className="grid max-w-full gap-x-6 gap-y-5 overflow-x-hidden xl:grid-cols-[minmax(0,1.15fr)_minmax(0,.85fr)]">{headlines.map((headline, index) => <NewspaperHeadline key={`${headline.title}-${headline.url}`} headline={headline} lead={index === 0} />)}</div>
